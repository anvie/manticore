...
import com.ansvia.manticore.algo.{ZZLegOp, FractalOp, Result, ManticoreAlgo}

/**
* Using leg matching algo (heur-3) menggunakan dice sorensen metric.
*/
class Fractal1(dataGen:DataGenerator, dataGenTarget:DataGenerator) 
    extends ManticoreAlgo(dataGen, dataGenTarget)
    with ZZLegOp
    with FractalOp {
...
//    }
    ...
    def calculate(posTimeStr:String) = {

        val ts = Util.parseTime(posTimeStr).getTime
        
        implicit lazy val currentData = dataGenTarget.chunkedData.filter(_.timestamp <= ts)

        val lastLeg = getLastLeg(ts)
        val uncompletedLeg = getUncompletedLeg(lastLeg.timestamp)
